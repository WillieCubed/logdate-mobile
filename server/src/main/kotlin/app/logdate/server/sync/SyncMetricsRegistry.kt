...
class SyncMetricsRegistry {
    ...
    fun recordOperation(name: String, durationMs: Long, success: Boolean, bytes: Long = 0L) {
    ...
    fun snapshot(): SyncMetricsSnapshot {
        val snapshots = operations.entries
            .sortedBy { it.key }
...
private class SyncOperationMetricsAccumulator {
    ...
    fun record(durationMs: Long, success: Boolean, bytes: Long) {
        if (success) {
    ...
    fun snapshot(name: String): SyncOperationMetricsSnapshot {
        return SyncOperationMetricsSnapshot(
            name = name,
