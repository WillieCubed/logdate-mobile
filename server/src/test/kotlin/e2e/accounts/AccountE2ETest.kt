...
class AccountE2ETest {
    ...
    fun `account creation begin with existing username behavior`() = testApplication {
        ...
        val beginResponse = client.post("/api/v1/accounts/create/begin") {
            contentType(ContentType.Application.Json)
            setBody("""
                {
                    "username": "$username",
        ...
        val completeResponse = client.post("/api/v1/accounts/create/complete") {
            contentType(ContentType.Application.Json)
            setBody("""
                {
    ...
    @Test
    fun `account creation complete with invalid session returns unauthorized`() = testApplication {
        application {
            module()
        }
    ...
    @Test
    fun `authentication complete with invalid session returns unauthorized`() = testApplication {
        application {
            module()
        }
        
        val response = client.post("/api/v1/accounts/authenticate/complete") {
            contentType(ContentType.Application.Json)
            setBody("""
                {
                    "credential": {
    ...
    @Test
    fun `token refresh with empty refresh token returns unauthorized`() = testApplication {
        application {
            module()
        }
    ...
    @Test
    fun `token refresh with invalid refresh token returns unauthorized`() = testApplication {
        application {
            module()
        }
    ...
    @Test
    fun `token refresh with missing refresh token field returns error`() = testApplication {
        application {
            module()
        }
