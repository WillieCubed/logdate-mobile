...
class EmailVerificationServiceTest {
    ...
    @Test
    fun `begin persists a single-use challenge`(): Unit = runBlocking {
    ...
    @Test
    fun `complete returns Success and attaches the verified email`(): Unit = runBlocking {
        val (service, _, accounts) = newService(
    ...
    @Test
    fun `complete refuses an unknown transaction id`(): Unit = runBlocking {
    ...
    @Test
    fun `complete refuses a second use of the same transaction id`(): Unit = runBlocking {
        val (service, _, accounts) = newService(
            verifier = stubVerifierAcceptingAnyNonce("u@example.com"),
        )
    ...
    @Test
    fun `complete refuses a challenge bound to a different account`(): Unit = runBlocking {
        val (service, _, accounts) = newService(
            verifier = stubVerifierAcceptingAnyNonce("u@example.com"),
        )
        accounts.save(accountFor(accountId))
    ...
    @Test
    fun `complete refuses an expired challenge`(): Unit = runBlocking {
    ...
    @Test
    fun `complete surfaces the verifier's reason code on signature failure`(): Unit = runBlocking {
        val (service, _, accounts) = newService(
    ...
    @Test
    fun `complete returns Conflict when email is already verified on another account`(): Unit = runBlocking {
        val (service, _, accounts) = newService(
    ...
    @Test
    fun `complete refuses when the requesting account does not exist`(): Unit = runBlocking {
        val (service, _, _) = newService(
    ...
    private inner class AcceptingVerifier(
        ...
        override suspend fun verify(credentialJson: String, expectedNonce: String): kotlin.Result<VerifiedEmailClaims> {
    ...
    private inner class RejectingVerifier(private val reason: String) : DigitalCredentialVerifier(
        jwksCache = trivialCache(),
        expectedAudience = expectedAudience,
        clock = { fixedNow },
    ) {
        override suspend fun verify(credentialJson: String, expectedNonce: String): kotlin.Result<VerifiedEmailClaims> =
            kotlin.Result.failure(VerificationException(reason))
    }

    private fun trivialCache(): GoogleVcJwksCache = GoogleVcJwksCache(
