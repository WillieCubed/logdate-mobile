...
class AudioSyncFlowTest {
    ...
    @Test
    fun `full audio sync flow - record and sync to phone`() = runTest {
    ...
    @Test
    fun `audio sync fails gracefully when phone not connected`() = runTest {
        val noteId = Uuid.random()
        val audioNote = JournalNote.Audio(
            uid = noteId,
            creationTimestamp = fixedTime,
    ...
    @Test
    fun `audio metadata syncs even when file transfer fails`() = runTest {
        val noteId = Uuid.random()
        val audioNote = JournalNote.Audio(
            uid = noteId,
            creationTimestamp = fixedTime,
    ...
    @Test
    fun `batch upload of multiple audio notes`() = runTest {
        val notes = (1..5).map { i ->
            val id = Uuid.random()
            id to JournalNote.Audio(
                uid = id,
        ...
        coEvery { syncMetadataService.getPendingUploads(EntityType.NOTE) } returns notes.map { (id, _) ->
    ...
    @Test
    fun `sync handles note deleted from DB between enqueue and upload`() = runTest {
        val noteId = Uuid.random()
    ...
    @Test
    fun `sync correctly handles delete operation in outbox`() = runTest {
        val noteId = Uuid.random()
    ...
    @Test
    fun `data sent to phone can reconstruct the exact original note`() = runTest {
        val noteId = Uuid.random()
        val originalNote = JournalNote.Audio(
            uid = noteId,
