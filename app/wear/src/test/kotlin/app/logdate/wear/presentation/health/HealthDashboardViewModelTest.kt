...
class HealthDashboardViewModelTest {
    ...
    private fun createViewModel(): HealthDashboardViewModel {
        return HealthDashboardViewModel(healthSensorManager, healthSnapshotDao)
    }
    ...
    @Test
    fun `initial state has no health data`() = runTest {
    ...
    @Test
    fun `loads current heart rate from sensor manager`() = runTest {
        coEvery { healthSensorManager.sampleCurrent() } returns HealthSnapshot(
            heartRateBpm = 72,
        )
        val viewModel = createViewModel()
    ...
    @Test
    fun `loads current step count from sensor manager`() = runTest {
        coEvery { healthSensorManager.sampleCurrent() } returns HealthSnapshot(
            stepCount = 8432,
        )
        val viewModel = createViewModel()
    ...
    @Test
    fun `loads both heart rate and step count`() = runTest {
        coEvery { healthSensorManager.sampleCurrent() } returns HealthSnapshot(
            heartRateBpm = 68,
            stepCount = 5000,
        )
    ...
    @Test
    fun `loads recent health snapshots from DAO`() = runTest {
    ...
    @Test
    fun `generates correlation insight from snapshots`() = runTest {
        val snapshots = listOf(
            HealthSnapshotEntity(
                id = Uuid.random(),
                noteId = Uuid.random(),
        ...
        assertTrue(viewModel.uiState.value.correlationInsight.isNotEmpty())
    }

    @Test
    fun `no correlation insight when no snapshots`() = runTest {
        coEvery { healthSnapshotDao.observeRecent(any()) } returns flowOf(emptyList())
        val viewModel = createViewModel()
        advanceUntilIdle()

        assertTrue(viewModel.uiState.value.correlationInsight.isEmpty())
    }
    ...
    @Test
    fun `handles sensor manager error gracefully`() = runTest {
