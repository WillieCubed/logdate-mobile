...
class GoogleWearDataLayerClientTest {
    ...
    @Test
    fun `putDataItem returns true when wearable data client succeeds`() = runTest {
    ...
    fun `deleteDataItem returns false when wearable delete fails`() = runTest {
    ...
    @Test
    fun `isPhoneConnected returns true when reachable phone capability exists`() = runTest {
        val node = mockk<Node>()
        val capabilityInfo =
    ...
    @Test
    fun `isPhoneConnected returns false when capability has no reachable nodes`() = runTest {
        val capabilityInfo =
            mockk<CapabilityInfo> {
    ...
    @Test
    fun `isPhoneConnected falls back to connected nodes when capability lookup fails`() = runTest {
        val node = mockk<Node>()
    ...
    @Test
    fun `isPhoneConnected returns false when capability and node lookups both fail`() = runTest {
        every { capabilityClient.getCapability(any(), any()) } returns Tasks.forException(IllegalStateException("boom"))
    ...
    @Test
    fun `getConnectedPhoneName returns capability node name when available`() = runTest {
        val node =
            mockk<Node> {
    ...
    @Test
    fun `getConnectedPhoneName returns null when capability lookup finds no nodes`() = runTest {
        val capabilityInfo =
            mockk<CapabilityInfo> {
                every { nodes } returns emptySet()
            }
    ...
    @Test
    fun `getConnectedPhoneName falls back to connected node when capability lookup fails`() = runTest {
        val node =
            mockk<Node> {
    ...
    @Test
    fun `getConnectedPhoneName returns null when capability and node lookup both fail`() = runTest {
        every { capabilityClient.getCapability(any(), any()) } returns Tasks.forException(IllegalStateException("boom"))
        every { nodeClient.connectedNodes } returns Tasks.forException(IllegalStateException("boom"))
    ...
    @Test
    fun `sendMessage returns true when a connected phone node accepts the message`() = runTest {
        val phoneNode =
            mockk<Node> {
    ...
    @Test
    fun `sendMessage returns false when no phone nodes are connected`() = runTest {
    ...
    @Test
    fun `sendMessage returns false when wearable message client throws`() = runTest {
        val phoneNode =
            mockk<Node> {
                every { id } returns "phone-node"
            }
        every { nodeClient.connectedNodes } returns Tasks.forResult(listOf(phoneNode))
        every { messageClient.sendMessage("phone-node", "/logdate/test", any()) } returns Tasks.forException(IllegalStateException("boom"))
    ...
    @Test
    fun `sendMessage returns false when connected node lookup throws`() = runTest {
        every { nodeClient.connectedNodes } returns Tasks.forException(IllegalStateException("boom"))
    ...
    @Test
    fun `downloadAudioFromPhone returns false when request message cannot be sent`() = runTest {
        every { nodeClient.connectedNodes } returns Tasks.forResult(emptyList())
    ...
    @Test
    fun `downloadAudioFromPhone returns false when transfer times out`() = runTest {
        ...
        every { messageClient.sendMessage("phone-node", WearAudioRequestPaths.audioRequestPath(noteId), any()) } returns Tasks.forResult(1)
    ...
    @Test
    fun `downloadAudioFromPhone writes file after matching transfer channel opens`() = runTest {
        val noteId = Uuid.random()
    ...
    @Test
    fun `downloadAudioFromPhone ignores unrelated channels before matching transfer arrives`() = runTest {
        val noteId = Uuid.random()
        val transferPath = WearAudioRequestPaths.audioTransferPath(noteId)
        val requestPath = WearAudioRequestPaths.audioRequestPath(noteId)
        val payload = "phone-synced-audio".encodeToByteArray()
    ...
    @Test
    fun `downloadAudioFromPhone deletes partial file when incoming stream fails`() = runTest {
        val noteId = Uuid.random()
        val transferPath = WearAudioRequestPaths.audioTransferPath(noteId)
        val requestPath = WearAudioRequestPaths.audioRequestPath(noteId)
        val callback = slot<ChannelClient.ChannelCallback>()
    ...
    @Test
    fun `sendFile copies local file bytes into wearable channel output stream`() = runTest {
    ...
    @Test
    fun `sendFile returns false when no phone nodes are connected`() = runTest {
        every { nodeClient.connectedNodes } returns Tasks.forResult(emptyList())
    ...
    @Test
    fun `sendFile closes channel and returns false when output stream lookup fails`() = runTest {
        val payload = "watch-upload".encodeToByteArray()
        val tempDir = createTempDirectory()
        val source = tempDir.resolve("local-note.m4a")
        source.writeBytes(payload)
