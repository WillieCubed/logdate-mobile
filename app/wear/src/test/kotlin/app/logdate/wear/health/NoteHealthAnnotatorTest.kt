...
class NoteHealthAnnotatorTest {
    ...
    private fun createAnnotator(): NoteHealthAnnotator = NoteHealthAnnotator(healthSensorManager, healthSnapshotDao)
    ...
    @Test
    fun `annotate saves snapshot with heart rate and step count`() =
        runTest {
    ...
    @Test
    fun `annotate persists entity to DAO`() =
        runTest {
            coEvery { healthSensorManager.sampleCurrent() } returns
                HealthSnapshot(
                    heartRateBpm = 72,
    ...
    @Test
    fun `annotate saves with only heart rate`() =
        runTest {
            coEvery { healthSensorManager.sampleCurrent() } returns
                HealthSnapshot(
                    heartRateBpm = 80,
    ...
    @Test
    fun `annotate saves with only step count`() =
        runTest {
            coEvery { healthSensorManager.sampleCurrent() } returns
                HealthSnapshot(
                    stepCount = 12000,
    ...
    @Test
    fun `annotate includes stress level when available`() =
        runTest {
            coEvery { healthSensorManager.sampleCurrent() } returns
                HealthSnapshot(
                    heartRateBpm = 90,
    ...
    @Test
    fun `annotate returns null when no health data available`() =
        runTest {
    ...
    @Test
    fun `annotate returns null when sensor manager throws`() =
        runTest {
    ...
    @Test
    fun `annotate returns null when DAO insert throws`() =
        runTest {
    ...
    @Test
    fun `annotate generates unique snapshot IDs`() =
        runTest {
            coEvery { healthSensorManager.sampleCurrent() } returns HealthSnapshot(heartRateBpm = 72)
            val annotator = createAnnotator()
