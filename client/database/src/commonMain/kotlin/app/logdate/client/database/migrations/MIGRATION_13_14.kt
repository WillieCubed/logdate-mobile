...
val MIGRATION_13_14 =
    object : Migration(13, 14) {
        override fun migrate(connection: SQLiteConnection) {
            ...
            connection.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS `index_indexed_media_images_mediaId` ON `indexed_media_images` (`mediaId`)",
            )
            ...
            // Create index on mediaId to enforce uniqueness and for foreign key lookup
            connection.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS `index_indexed_media_videos_mediaId` ON `indexed_media_videos` (`mediaId`)",
            )
            ...
            connection.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS `index_rewind_generation_requests_startTime_endTime` ON `rewind_generation_requests` (`startTime`, `endTime`)",
            )
        }
    }
