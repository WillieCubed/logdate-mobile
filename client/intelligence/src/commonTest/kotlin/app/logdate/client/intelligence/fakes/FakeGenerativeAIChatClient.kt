...
class FakeGenerativeAIChatClient : GenerativeAIChatClient {
    ...
    fun getLastUserMessage(): String? = submissions.lastOrNull()?.find { it.role == "user" }?.content

    fun getLastSystemMessage(): String? = submissions.lastOrNull()?.find { it.role == "system" }?.content
}
