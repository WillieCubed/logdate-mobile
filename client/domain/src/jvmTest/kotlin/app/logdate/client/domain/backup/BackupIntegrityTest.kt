...
class BackupIntegrityTest {
    ...
    private val createBackup = CreateEncryptedBackupUseCase(
    ...
    @Test
    fun `crypto primitives work`() = runBlocking {
        val recoveryPhrase = listOf("witch", "collapse", "practice", "feed", "shame", "open", "despair", "creek", "road", "again", "ice", "least")
    ...
    @Test
    fun `e2e encryption and decryption preserves data integrity`() = runBlocking {
        val backupPath = "/backup.enc".toPath()
        val recoveryPhrase = listOf("witch", "collapse", "practice", "feed", "shame", "open", "despair", "creek", "road", "again", "ice", "least")
        val secretData = "This is the user's soul. It must be protected at all costs."

        // 1. Create Encrypted Backup
        val progress = createBackup(backupPath, recoveryPhrase) { sink ->
        ...
        val decryptedSource = cryptoManager.decryptSource(
        ...
        try {
            val corruptedSource = cryptoManager.decryptSource(
                fileSystem.source(backupPath),
                masterKey,
                deterministicIv
            )
