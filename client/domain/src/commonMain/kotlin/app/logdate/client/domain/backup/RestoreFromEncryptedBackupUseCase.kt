...
class RestoreFromEncryptedBackupUseCase(
    ...
    operator fun invoke(backupFile: Path, recoveryPhrase: List<String>): Flow<RestoreProgress> = flow {
     ...
     */
    private suspend fun deriveKeys(recoveryPhrase: List<String>): ByteArray {
        return cryptoManager.deriveMasterKey(recoveryPhrase)
    }

    /**
     * Connects the encrypted source to the restoration logic and performs integrity checks.
     */
    private fun performRestore(backupFile: Path, masterKey: ByteArray, iv: ByteArray) {
