...
class SearchFiltersTest {
    ...
    fun `AllTime returns sentinel distant-past to distant-future window`() {
        val window = DateRangeFilter.AllTime.window(now, tz)
    ...
    fun `Today covers from start of local day to start of next day`() {
        val window = DateRangeFilter.Today.window(now, tz)
    ...
    @Test
    fun `ThisWeek starts Monday and is exactly seven days long`() {
        val window = DateRangeFilter.ThisWeek.window(now, tz)
        // 2026-05-04 is a Monday, so the week starts the same day.
        assertEquals(LocalDate(2026, 5, 4).atStartOfDayIn(tz), window.from)
        assertEquals(LocalDate(2026, 5, 11).atStartOfDayIn(tz), window.toExclusive)
    }

    @Test
    fun `ThisWeek backs up to Monday when now is later in the week`() {
        // Sunday 2026-05-10 — Monday should be 2026-05-04
        val sunday = LocalDateTime(2026, 5, 10, 23, 0).toInstant(tz)
        val window = DateRangeFilter.ThisWeek.window(sunday, tz)
        assertEquals(LocalDate(2026, 5, 4).atStartOfDayIn(tz), window.from)
        assertEquals(LocalDate(2026, 5, 11).atStartOfDayIn(tz), window.toExclusive)
    }
    ...
    fun `ThisMonth covers the calendar month in local timezone`() {
        val window = DateRangeFilter.ThisMonth.window(now, tz)
    ...
    fun `ThisYear covers Jan 1 to Jan 1 next year in local timezone`() {
        val window = DateRangeFilter.ThisYear.window(now, tz)
