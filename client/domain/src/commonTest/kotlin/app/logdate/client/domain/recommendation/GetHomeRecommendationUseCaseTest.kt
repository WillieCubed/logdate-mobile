...
class GetHomeRecommendationUseCaseTest {
    ...
    fun setUp() {
        ...
        useCase = GetHomeRecommendationUseCase(
    ...
    @Test
    fun `returns None when user has notes today and no drafts`() = runTest {
    ...
    @Test
    fun `returns CaptureToday when user has no notes today and no drafts`() = runTest {
    ...
    @Test
    fun `CaptureToday carries default message`() = runTest {
        mockNotesRepository.notesForRange = emptyList()
        mockDraftRepository.drafts = emptyList()
    ...
    @Test
    fun `returns CompleteYourDraft when a draft exists and no notes today`() = runTest {
    ...
    @Test
    fun `CompleteYourDraft draft ID matches the draft`() = runTest {
    ...
    @Test
    fun `CompleteYourDraft preview comes from first text note in draft`() = runTest {
    ...
    @Test
    fun `CompleteYourDraft preview is null when draft has no text notes`() = runTest {
    ...
    @Test
    fun `CompleteYourDraft takes priority over CaptureToday when draft exists`() = runTest {
    ...
    @Test
    fun `CompleteYourDraft takes priority even when user already has notes today`() = runTest {
        val draft = createDraftWithText("Draft content")
    ...
    @Test
    fun `emits updated recommendation when notes are added`() = runTest {
    ...
    @Test
    fun `emits updated recommendation when draft is created`() = runTest {
    ...
    @Test
    fun `emits None when draft is deleted and notes exist`() = runTest {
    ...
    private fun createTextNote(content: String = "Test note") = JournalNote.Text(
        uid = Uuid.random(),
        content = content,
    ...
    private class MockJournalNotesRepository : JournalNotesRepository {
        ...
        override fun observeNotesInRange(start: Instant, end: Instant): Flow<List<JournalNote>> =
            observeRangeFlow ?: flowOf(notesForRange)
