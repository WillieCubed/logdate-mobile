...
class GetAccountSetupDataUseCaseTest {
    ...
    private class MockKeyValueStorage : KeyValueStorage {
        ...
        override fun observeInt(
            key: String,
            defaultValue: Int,
        ): Flow<Int> = intFlows.getOrPut(key) { MutableStateFlow((store[key] as? Int) ?: defaultValue) }.asStateFlow()

        override fun observeLong(
            key: String,
            defaultValue: Long,
        ): Flow<Long> = longFlows.getOrPut(key) { MutableStateFlow((store[key] as? Long) ?: defaultValue) }.asStateFlow()

        override fun observeFloat(
            key: String,
            defaultValue: Float,
        ): Flow<Float> = floatFlows.getOrPut(key) { MutableStateFlow((store[key] as? Float) ?: defaultValue) }.asStateFlow()
    }
}
