...
class IosMediaManager(
    ...
    override suspend fun getMedia(uri: String): MediaObject = withContext(Dispatchers.Default) {
    ...
    override suspend fun exists(mediaId: String): Boolean = withContext(Dispatchers.Default) {
        val path = resolvePath(mediaId)
        path != null && fileManager.fileExistsAtPath(path)
    }

    override suspend fun getRecentMedia(): Flow<List<MediaObject>> = withContext(Dispatchers.Default) {
    ...
    override suspend fun addToDefaultCollection(uri: String) = withContext(Dispatchers.Default) {
    ...
    override suspend fun readMedia(uri: String): MediaPayload = withContext(Dispatchers.Default) {
    ...
    override suspend fun saveMedia(payload: MediaPayload): String = withContext(Dispatchers.Default) {
        ensureMediaDir()
    ...
    private fun listMediaObjects(): List<MediaObject> {
        ...
        return files.filterIsInstance<String>()
    ...
    private fun toMediaObject(path: String): MediaObject? {
        ...
        val timestamp = modifiedAt?.let { date ->
            Instant.fromEpochMilliseconds((date.timeIntervalSince1970 * 1000).toLong())
        } ?: Instant.fromEpochMilliseconds(0)

        return when {
            imageExtensions.contains(extension) -> MediaObject.Image(
                uri = "file://$path",
                size = size.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(),
                name = name,
                timestamp = timestamp
            )
            videoExtensions.contains(extension) -> MediaObject.Video(
                name = name,
                uri = "file://$path",
                size = size.coerceAtMost(Int.MAX_VALUE.toLong()).toInt(),
                timestamp = timestamp,
    ...
    private fun buildMediaPath(fileName: String): String {
