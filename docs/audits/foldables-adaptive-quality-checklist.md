...
| `[partial]` | Watch settings and troubleshooting | Watch settings and troubleshooting now use `FoldableBookLayout` so device status/privacy, sync/notification actions, troubleshooting actions, and guidance can split across vertical hinges. Runtime evidence is still missing. | `WatchSettingsRoute`, `WatchTroubleshootingRoute`, `WatchEntries.kt` |
...
| `[partial]` | Watch settings and troubleshooting | Watch settings/troubleshooting routes are registered through `watchEntries`, and both screens now use `FoldableBookLayout` for status/action/guidance separation in book posture. | Add folded screenshots and validate sync, notification, association, install, open-on-watch, and troubleshooting return behavior. |
