...
| `[partial]` | Search and search result sharing | Search opens many detail routes and exposes Android share UI. The expanded search body now splits filters/context from recents/results in vertical-hinge book posture, with idle/searching/empty/results screenshot evidence. Share-sheet return evidence remains open. | `SearchRoute`, `SearchScreen`, `MainActivity.shareSearchResult`; `A30_SearchIdleBookPosture` through `A33_SearchResultsBookPosture`; `validateDebugScreenshotTest --tests '*AdaptiveSearchBookPosture*'` |
| `[partial]` | Sync issues screen | Notification or sync entry opens an operational queue that now has a vertical-hinge split layout and book-posture screenshot evidence. Fold/unfold return and multi-window entry evidence remain open. | `SyncIssuesRoute`, `SyncIssuesScreen`; `A29_SyncIssuesBookPosture`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
...
| `[partial]` | Events settings | Events hub now uses `FoldableBookLayout` to separate the event toggle/smart naming controls from calendar and sync navigation, but runtime evidence is still missing. | `EventsSettingsRoute`, `EventsSettingsScreen` |
| `[partial]` | Calendar sync settings, calendars, and activity | Calendar sync settings now use `FoldableBookLayout` to separate permission/toggle controls from calendar and activity navigation, but calendars and activity detail/runtime evidence still need validation. | `CalendarSyncSettingsRoute`, `CalendarSyncCalendarsRoute`, `CalendarSyncActivityRoute` |
...
| Status | Feature or flow | Why it needs the checklist | Route or code evidence |
| --- | --- | --- | --- |
| `[partial]` | Settings overview | Dense navigation hub uses `FoldableBookLayout`; book-posture screenshot validation passes for the vertical-hinge split. Compact, expanded, and settings handoff return evidence remain open. | `SettingsRoute`, `SettingsOverviewScreen`; `AdaptiveSettingsBookPostureScreenshots.kt`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
| `[partial]` | Account, privacy, passkeys, and recovery phrase settings | Account and privacy settings use `FoldableBookLayout`; book-posture screenshot validation passes with deterministic passkey timestamps. Recovery phrase modal/return validation remains open. | `AccountSettingsRoute`, `PrivacySettingsRoute`; `AdaptiveSettingsBookPostureScreenshots.kt`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
| `[partial]` | Data, export, restore, cloud backup, and clear/reset data settings | Data settings use `FoldableBookLayout`; book-posture screenshot validation passes for the settings hub. File picker, export/restore, and destructive return-state evidence remain open. | `DataSettingsRoute`, `ExportSettingsRoute`, `ResetSettingsRoute`, `ClearDataSettingsRoute`, `ResetAppSettingsRoute`; `AdaptiveSettingsBookPostureScreenshots.kt`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
| `[partial]` | Devices settings | Device list and reset action use `FoldableBookLayout`; book-posture screenshot validation passes. Rename, remove, reset, and multi-window return flows remain open. | `DevicesRoute`, `DevicesScreen`; `AdaptiveSettingsBookPostureScreenshots.kt`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
| `[partial]` | Location settings and sub-settings | Location options can be reached from notifications and the main/advanced location screens now use `FoldableBookLayout`, but interval and timeline subpages still need posture evidence. | `LocationSettingsRoute`, `LocationTrackingOptionsRoute`, `LocationIntervalRoute`, `LocationAdvancedRoute` |
| `[partial]` | Memories, voice notes, streak, timeline, day boundary, sync, library, recommendation, birthday, rewind, and advanced settings | These preference pages use `FoldableBookLayout`; book-posture screenshot validation now passes for memories, voice notes, streaks, timeline, day boundary, sync, library, recommendations, birthday, rewind, and advanced settings. Compact/expanded and dialog return evidence remain open. | `MemoriesSettingsRoute`, `VoiceNotesSettingsRoute`, `StreakSettingsRoute`, `TimelineSettingsRoute`, `DayBoundarySettingsRoute`, `SyncSettingsRoute`, `LibrarySettingsRoute`, `RecommendationSettingsRoute`, `BirthdaySettingsRoute`, `RewindSettingsRoute`, `AdvancedSettingsRoute`; `AdaptiveSettingsBookPostureScreenshots.kt`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
| `[partial]` | Watch settings and troubleshooting | Watch settings, sync settings, notification settings, and troubleshooting now use `FoldableBookLayout` or the shared settings scaffold so device status/privacy, sync/notification actions, troubleshooting actions, and guidance can split across vertical hinges. Book-posture screenshot validation passes for the watch hub and detail pages. Device/action return flows remain open. | `WatchSettingsRoute`, `WatchSyncSettingsRoute`, `WatchNotificationSettingsRoute`, `WatchTroubleshootingRoute`, `WatchEntries.kt`; `AdaptiveSettingsBookPostureScreenshots.kt`; `validateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
...
| `[partial]` | Runtime validation | Book-posture screenshot validation now passes for 25 previews: 21 settings and settings-adjacent previews from `A09_SettingsOverviewBookPosture` through `A29_SyncIssuesBookPosture`, plus search previews `A30_SearchIdleBookPosture` through `A33_SearchResultsBookPosture`. Folded/unfolded emulator, tabletop, PiP, attachment, and multi-instance evidence remain open. | Keep expanding posture screenshots and add safe emulator/managed-device commands plus manual hardware notes. Do not use physical devices from agent work without an explicit current-conversation override. |
...
| `[partial]` | Search and search result sharing | Search routes to many detail screens and Android share works through chooser. `SearchScreenContent` now uses `FoldableBookLayout` in vertical-hinge book posture so filters/context stay on one physical pane while recents, progress, empty states, or result buckets stay on the other. Book-posture screenshot validation passes for `A30_SearchIdleBookPosture` through `A33_SearchResultsBookPosture`. | Add share-sheet return validation and result-to-detail route restoration from folded/unfolded and multi-window modes. |
| `[partial]` | Sync issues screen | Operational list screen now uses `FoldableBookLayout` so the issue summary/counts stay on one physical pane and the retry/discard queue stays on the other. Book-posture screenshot validation passes for `A29_SyncIssuesBookPosture`. | Add folded/unfolded entry validation from the banner/notification path and multi-window retry/discard return screenshots. |
...
| `[partial]` | Journals overview and empty journals state | Journal browsing has adaptive panel shape and search/filter UI. `JournalsOverviewScreenContent` now uses `FoldableBookLayout` so search/suggestions stay on one vertical-hinge pane and the journal list/filter panel fills the other pane. | Add runtime book-posture screenshots for populated, empty, search, and suggestion states. |
...
| `[partial]` | Note viewer | Text/audio/video note viewer exists; video uses the shared `VideoPlayerContent`, including Android video PiP entry. `NoteViewerScaffoldContent` now uses `FoldableTabletopLayout` so note content stays above a horizontal hinge and back/share/add-to-journal/previous/next controls move below it. The standard non-tabletop path uses `FoldableBookLayout` so vertical-hinge book posture keeps note content on one physical pane and navigation/actions on the other. | Add runtime validation for text, image, and video notes in book and tabletop posture; validate video PiP entry/return from the note viewer. |
...
| `[partial]` | Rewind detail story | Immersive story panels and notification deep links exist. `RewindStoryView` now uses `FoldableTabletopLayout` so the active story panel stays above a horizontal hinge while progress, share, reply, delete, close, previous, and next controls move below it. The standard non-tabletop path uses `FoldableBookLayout` so vertical-hinge book posture keeps the story panel on one physical pane and chrome/navigation on the other. Rewind remains immersive across screen sizes, with only a large-screen max width to keep the story stage and chrome readable. | Add runtime tabletop/book screenshots and test notification deep link into folded Rewind detail. |
...
| `[partial]` | Events settings | Events settings now use `FoldableBookLayout` for the hub content and navigation split, but runtime evidence is still missing. | Record folded/unfolded screenshots and validate navigation return from the split layout. |
| `[partial]` | Calendar sync settings, calendars, and activity | Calendar-sync settings now use `FoldableBookLayout` for permission/toggle vs. navigation separation, but calendars and activity screens still need posture evidence. | Add split list/detail activity layout and validate event-detail navigation from folded multi-window mode. |
...
| `[partial]` | Settings overview | Dense navigation hub uses `FoldableBookLayout` to split personal, privacy/security, and data/storage groups across vertical hinges. Book-posture screenshot validation passes for `A09_SettingsOverviewBookPosture`. | Remaining work: compact/expanded/small-pane screenshot evidence and notification row/system-settings return validation. |
| `[partial]` | Account, privacy, passkeys, and recovery phrase settings | Account and privacy screens use `FoldableBookLayout`. Book-posture screenshot validation passes for `A10_AccountSettingsBookPosture` and `A11_PrivacySettingsBookPosture` after pinning passkey timestamps in shared screenshot data. | Remaining work: recovery phrase modal placement, passkey create/revoke dialog return, and fold/resize state validation. |
| `[partial]` | Data, export, restore, cloud backup, and clear/reset data settings | Data settings use `FoldableBookLayout`. Book-posture screenshot validation passes for `A12_DataSettingsBookPosture`. | Remaining work: file picker return, sheet/dialog state, export/restore progress UI, and destructive confirmation validation. |
| `[partial]` | Devices settings | Device list and reset action use `FoldableBookLayout`. Book-posture screenshot validation passes for `A14_DevicesSettingsBookPosture`. | Remaining work: rename, remove, reset, folded, and multi-window return validation. |
| `[partial]` | Location settings and sub-settings | Location settings now use `FoldableBookLayout` for the main hub and advanced settings, but interval and timeline subpages still need posture evidence. | Add split settings/detail layout and validate notification/location-history entry return. |
| `[partial]` | Memories, voice notes, streak, timeline, day boundary, sync, library, recommendation, birthday, rewind, and advanced settings | These preference pages split preference summaries, primary opt-ins, dense controls, and supporting actions with `FoldableBookLayout`. Book-posture screenshot validation passes for `A13_MemoriesSettingsBookPosture`, `A15_RewindSettingsBookPosture`, and `A20_StreakSettingsBookPosture` through `A28_SyncSettingsBookPosture`. | Remaining work: compact/expanded posture screenshots and dialog return state for recommendation times and birthday editing. |
| `[partial]` | Watch settings and troubleshooting | Watch routes are registered through `watchEntries`. Book-posture screenshot validation passes for `A16_WatchSettingsBookPosture`, `A17_WatchSyncSettingsBookPosture`, `A18_WatchNotificationSettingsBookPosture`, and `A19_WatchTroubleshootingBookPosture`. | Remaining work: validate sync, notification, association, install, open-on-watch, and troubleshooting return behavior on a foldable emulator or device-safe manual run. |
