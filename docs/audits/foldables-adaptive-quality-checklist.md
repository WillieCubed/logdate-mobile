...
| `[partial]` | Location settings and sub-settings | Location options can be reached from notifications and the main/advanced location screens now use `FoldableBookLayout`, but interval and device-specific subpages still need posture evidence. | `LocationSettingsRoute`, `LocationTrackingOptionsRoute`, `LocationIntervalRoute`, `LocationAdvancedRoute` |
...
| `[partial]` | Location settings and sub-settings | Location settings now use `FoldableBookLayout` for the main hub and advanced settings, but interval and device-specific subpages still need posture evidence. | Add split settings/detail layout and validate notification/location-history entry return. |
