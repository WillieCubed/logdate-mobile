...
| `[partial]` | Memories, voice notes, streak, timeline, day boundary, sync, library, recommendation, birthday, rewind, and advanced settings | Voice notes, streak, timeline, day boundary, library, and advanced settings now split preference summaries and controls with `FoldableBookLayout`. Memories, sync, recommendation, birthday, Rewind settings, and runtime evidence remain open. | Standardize the remaining pages on the shared settings scaffold and add one screenshot state per page at compact, book, and expanded widths. |
...
| `[partial]` | Foldables_Postures | Book posture gives long text and reading-heavy flows a readable layout, such as single-side reading, dual-pane reading/detail, or hinge-aware gutters. | Home, `AdaptivePaneLayout`, and `FoldableBookLayout` can split around vertical hinges. Journals overview, note viewer, library media detail, Rewind detail, settings overview, account settings, privacy settings, data settings, devices settings, location settings, timeline settings, voice notes settings, streak settings, day boundary settings, library settings, and advanced settings now use `FoldableBookLayout` for content/action separation; other reader-heavy feature screens and runtime evidence are still missing. |
