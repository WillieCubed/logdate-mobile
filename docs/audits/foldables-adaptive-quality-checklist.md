...
| `[fail]` | Tabletop posture | Media, audio, camera, and recording controls are placed by generic top/bottom alignment or width classes, not by horizontal hinge posture. | Add tabletop-specific layout branches for camera, audio recording/playback, media viewer, note viewer, and Rewind so controls move to the reachable horizontal screen area. |
| `[partial]` | Book posture | Home list/detail and shared `AdaptivePaneLayout` now have vertical-hinge split handling, but many reading-heavy screens still only constrain width or use width-based panes. | Add book-posture reader layouts for journals, note viewer, timeline day detail, Rewind detail, settings, onboarding, and long transcript surfaces. |
...
| `[partial]` | Video playback | Journal note viewing and library media detail render video through Android `VideoPlayerContent`, backed by Media3 `ExoPlayer` and `PlayerView`. Playback pauses on lifecycle pause/stop and exposes no PiP entry. | Audit video playback for foldable layout, tabletop controls, lifecycle, and resize behavior. Decide separately whether minimized/background video is a product requirement. |
| `[partial]` | PiP | Video playback now exposes explicit Android PiP entry from the shared `VideoPlayerContent`, keeps playback alive when the host activity enters PiP, and enables Android 12+ auto-enter while playing. Audio no longer exposes stale PiP callbacks. | Add runtime validation for note videos, library videos, and editor video blocks in folded, unfolded, portrait, landscape, and multi-window states; add custom PiP playback actions if product testing shows platform media controls are insufficient. |
...
| `[partial]` | Audio recording blocks | Recording UI and foreground notification support exist; control placement is not tabletop-aware. | Add tabletop layout that pins recording controls to one physical pane and waveform/status to the other. |
| `[partial]` | Audio playback blocks and immersive audio | Playback services and immersive audio UI exist; controls are not posture-aware. Audio PiP callback scaffolding has been removed because PiP is scoped to video playback. | Add wide/tabletop immersive audio layouts with hinge-safe transport controls and validate notification controls. |
...
| `[partial]` | Camera/photo/video capture blocks | Inline CameraX preview, camera switch, aspect ratios, and review states exist, but no foldable camera behavior. | Add posture-specific preview/control placement, rotation verification, and product decision for front/back screen preview support. |
| `[partial]` | Media attachment import and block rendering | Attachments are imported into editor blocks; placement/resizing across hinge states is unverified. | Add attachment block screenshot states for folded, unfolded, landscape, and editor multi-window. |
...
| `[partial]` | Note viewer | Text/audio/video note viewer exists; video uses the shared `VideoPlayerContent`, including Android video PiP entry. Reader posture is incomplete. | Add book-posture text/media layouts and preserve journal/navigation context across posture changes; validate video PiP entry/return from the note viewer. |
...
| `[partial]` | Media detail viewer | Expanded media detail uses side-by-side metadata based on width and renders videos through `VideoPlayerContent`, including Android video PiP entry. Hinge integration is incomplete. | Place media and metadata on physical panes when hinge exists; validate video PiP entry/return from library media detail. |
...
| `[partial]` | Rewind detail story | Immersive story panels and notification deep links exist; no posture-aware story controls. | Add hinge-safe story navigation and tabletop/media panel placement; test notification deep link into folded Rewind detail. |
...
| `[fail]` | Foldables_Postures | Tabletop posture moves media playback, recording, or review controls to the reachable horizontal screen area instead of spanning the fold. | No tabletop-specific control placement found for audio, media, camera, or Rewind. |
| `[partial]` | Foldables_Postures | Book posture gives long text and reading-heavy flows a readable layout, such as single-side reading, dual-pane reading/detail, or hinge-aware gutters. | Home and `AdaptivePaneLayout` can split around vertical hinges; reader-heavy feature screens still need migration and runtime evidence. |
