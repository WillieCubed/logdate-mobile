...
| `[partial]` | Rewind detail story | Immersive story panels and notification deep links exist. `RewindStoryView` now uses `FoldableTabletopLayout` so the active story panel stays above a horizontal hinge while progress, share, reply, delete, close, previous, and next controls move below it. Rewind remains immersive across screen sizes, with only a large-screen max width to keep the story stage and chrome readable. | Add runtime tabletop screenshots and test notification deep link into folded Rewind detail. |
