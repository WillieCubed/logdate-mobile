...
| `[partial]` | Book posture | Home list/detail and shared `AdaptivePaneLayout` now have vertical-hinge split handling. `FoldableBookLayout` provides reusable vertical-hinge panes. Journals overview, note viewer, library media detail, and Rewind detail use it to keep controls or metadata on one physical pane and primary browsing/viewing content on the other. Many reading-heavy screens still only constrain width or use width-based panes. | Add book-posture reader layouts for journal detail, timeline day detail, settings, onboarding, and long transcript surfaces; add runtime foldable screenshots before marking pass. |
