...
| `[partial]` | Book posture | Home list/detail and shared `AdaptivePaneLayout` now have vertical-hinge split handling. `FoldableBookLayout` provides reusable vertical-hinge panes, and the note viewer uses it to keep note content on one physical pane with navigation/actions on the other. Many reading-heavy screens still only constrain width or use width-based panes. | Add book-posture reader layouts for journals, timeline day detail, Rewind detail, settings, onboarding, and long transcript surfaces; add runtime foldable screenshots before marking pass. |
...
| `[partial]` | Foldables_Postures | Book posture gives long text and reading-heavy flows a readable layout, such as single-side reading, dual-pane reading/detail, or hinge-aware gutters. | Home, `AdaptivePaneLayout`, and `FoldableBookLayout` can split around vertical hinges. Note viewer now uses `FoldableBookLayout` for content/action separation; other reader-heavy feature screens and runtime evidence are still missing. |
