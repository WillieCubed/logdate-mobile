...
| `[partial]` | Location settings and sub-settings | Location options can be reached from notifications, and the main, tracking options, interval, and advanced location settings screens now use `FoldableBookLayout` with book-posture screenshot evidence. Location timeline and entry/return validation remain open. | `LocationSettingsRoute`, `LocationTrackingOptionsRoute`, `LocationIntervalRoute`, `LocationAdvancedRoute`; `A30_LocationSettingsBookPosture` through `A33_LocationAdvancedBookPosture`; `updateDebugScreenshotTest --tests '*AdaptiveSettingsBookPosture*'` |
...
| `[partial]` | Location settings and sub-settings | Location settings now use `FoldableBookLayout` for the main hub, tracking options, interval, and advanced settings. Generated book-posture baselines cover `A30_LocationSettingsBookPosture` through `A33_LocationAdvancedBookPosture`. | Remaining work: location timeline layout, notification/location-history entry return, and folded/multi-window runtime validation. |
