...
| `[partial]` | Foldables_Postures | Book posture gives long text and reading-heavy flows a readable layout, such as single-side reading, dual-pane reading/detail, or hinge-aware gutters. | Home, `AdaptivePaneLayout`, and `FoldableBookLayout` can split around vertical hinges. Note viewer and Rewind detail now use `FoldableBookLayout` for content/action separation; other reader-heavy feature screens and runtime evidence are still missing. |
