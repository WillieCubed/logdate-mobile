...
| Status | Feature or flow | Why it needs the checklist | Route or code evidence |
| --- | --- | --- | --- |
| `[partial]` | Settings overview | Dense navigation hub now uses `FoldableBookLayout`; personal, privacy/security, and data/storage groups can split across vertical hinges. | `SettingsRoute`, `SettingsOverviewScreen` |
| `[partial]` | Account, privacy, passkeys, and recovery phrase settings | Sensitive forms and dialogs now use `FoldableBookLayout` in account and privacy settings, but runtime validation remains. | `AccountSettingsRoute`, `PrivacySettingsRoute` |
| `[partial]` | Data, export, restore, cloud backup, and clear/reset data settings | Long-running work, file pickers, and confirmation flows now use `FoldableBookLayout` in data settings, but runtime return-state evidence remains. | `DataSettingsRoute`, `ExportSettingsRoute`, `ResetSettingsRoute`, `ClearDataSettingsRoute`, `ResetAppSettingsRoute` |
| `[partial]` | Devices settings | Device list and reset action now use `FoldableBookLayout`, but runtime split-window evidence is still missing. | `DevicesRoute`, `DevicesScreen` |
...
| `[partial]` | Memories, voice notes, streak, timeline, day boundary, sync, library, recommendation, birthday, rewind, and advanced settings | These preference pages now use `FoldableBookLayout` so summaries, primary toggles, dense controls, and navigation/action groups can split across vertical hinges. Runtime posture evidence is still missing. | `MemoriesSettingsRoute`, `VoiceNotesSettingsRoute`, `StreakSettingsRoute`, `TimelineSettingsRoute`, `DayBoundarySettingsRoute`, `SyncSettingsRoute`, `LibrarySettingsRoute`, `RecommendationSettingsRoute`, `BirthdaySettingsRoute`, `RewindSettingsRoute`, `AdvancedSettingsRoute` |
...
| `[partial]` | Settings overview | Dense navigation hub now uses `FoldableBookLayout` to split personal, privacy/security, and data/storage groups across vertical hinges. | Add runtime screenshots for compact, expanded, book posture, and small-pane fallback; keep notification row and subpage return behavior validated. |
| `[partial]` | Account, privacy, passkeys, and recovery phrase settings | Sensitive settings and dialogs now use `FoldableBookLayout` in account and privacy screens, but modal placement and runtime validation still need evidence. | Add passkey/recovery phrase resize validation and record book-posture screenshots for account and privacy flows. |
| `[partial]` | Data, export, restore, cloud backup, and clear/reset data settings | Long-running export/restore and file picker flows now use `FoldableBookLayout` for data settings, but runtime return-state evidence is still missing. | Add restoration tests for file picker return, sheet/dialog state, progress UI, and destructive confirmations. |
| `[partial]` | Devices settings | Device list and reset action now use `FoldableBookLayout`, but split-window runtime evidence is still missing. | Add folded and multi-window screenshots for the list, rename, remove, and reset flows. |
...
| `[partial]` | Memories, voice notes, streak, timeline, day boundary, sync, library, recommendation, birthday, rewind, and advanced settings | These preference pages now split preference summaries, primary opt-ins, dense controls, and supporting actions with `FoldableBookLayout`. Runtime evidence remains open. | Add one screenshot state per page at compact, book, and expanded widths, including dialog return state for recommendation times and birthday editing. |
